...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='train', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        self.temp = "demo/2011_09_26_drive_0014_sync/2011_09_26/2011_09_26_drive_0014_sync"
        self.image_dir = os.path.join(self.dataset_dir, self.temp, "image_02")
        self.lidar_dir = os.path.join(self.dataset_dir, self.temp, "velodyne")
        self.calib_dir = os.path.join(self.dataset_dir, self.temp, "calib")
        self.label_dir = os.path.join(self.dataset_dir, self.temp, "label_2")
        split_txt_path = os.path.join(self.dataset_dir, 'ImageSets', '{}.txt'.format(mode))
        # self.sample_id_list = [int(x.strip()) for x in open(split_txt_path).readlines()]
        self.sample_id_list = sorted(glob(os.path.join(self.lidar_dir, '*.bin')))
        self.sample_id_list = [float(os.path.basename(fn)[-12:-4]) for fn in self.sample_id_list]
    ...
    def get_image(self, idx):
        ...
        img_path = os.path.join(self.image_dir, '0101_light_clear_smooth_{:08d}.png'.format(idx))
